# -*- coding: utf-8 -*-

import os
import time
import config

from providers.provider_factory import ProviderFactory
from providers.provider_priority_engine import ProviderPriorityEngine
from providers.provider_health_engine import ProviderHealthEngine
from providers.provider_retry_engine import ProviderRetryEngine
from providers.provider_rate_limit_engine import ProviderRateLimitEngine


class ContentRecoveryProvider:
    VERSION = "1.0.0"

    def generate(self, prompt):
        return {
            "content": (
                "هذا القسم العلمي يشرح الموضوع البيطري بشكل موسوعي "
                "اعتماداً على المعرفة العلمية المتاحة. "
                "يشمل التعريف، الأسباب، الأعراض، التشخيص، العلاج، "
                "والوقاية مع التركيز على التطبيق العملي في المزارع."
            )
        }


class ProviderManager:
    _instance = None

    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(ProviderManager, cls).__new__(cls)
        return cls._instance

    def __init__(self, provider_name=None):
        if getattr(self, "_initialized", False):
            return

        if not provider_name:
            provider_name = getattr(config, "ACTIVE_PROVIDER", None)

        if not provider_name:
            if os.getenv("OPENROUTER_API_KEY"):
                provider_name = "openrouter"
            elif os.getenv("GROQ_API_KEY"):
                provider_name = "groq"
            elif os.getenv("GEMINI_API_KEY"):
                provider_name = "gemini"
            else:
                provider_name = "groq"

        self.provider_name = provider_name
        self.provider = ProviderFactory.create(provider_name)

        self._cache = {}
        self._failed_providers = {}
        self._provider_cooldown = {}
        self._cooldown_seconds = 300
        self._min_content_length = 200
        self.health_engine = ProviderHealthEngine()
        self.retry_engine = ProviderRetryEngine()
        self.retry_config = self.retry_engine.retry_config()
        self.rate_limit_engine = ProviderRateLimitEngine()
        self.rate_limit_config = self.rate_limit_engine.limit()

        self._initialized = True

    def get_provider(self):
        return self.provider

    def set_provider(self, provider_name):
        self.provider_name = provider_name
        self.provider = ProviderFactory.create(provider_name)

    def _is_valid_content(self, content):
        if not content or not isinstance(content, str):
            return False
        stripped = content.strip()
        if len(stripped) < self._min_content_length:
            return False
        if len(set(stripped)) < 10:
            return False
        return True

    def generate(self, prompt):
        cache_key = str(hash(prompt))
        if cache_key in self._cache:
            print("♻️ Provider cache hit")
            return self._cache[cache_key]

        priority = ProviderPriorityEngine()
        providers = [self.provider_name]
        providers.extend(priority.priorities())
        providers = list(dict.fromkeys(providers))
        providers = providers

        for name in providers:
            try:
                health = self.health_engine.check(name)
                if not health.get("available", False):
                    self._failed_providers[name] = {
                        "error": "provider_unavailable",
                        "status": "health_failed",
                    }
                    continue

                provider = ProviderFactory.create(name)
                result = provider.generate(prompt)

                if isinstance(result, dict):
                    if result.get("success"):
                        content = result.get("content", "")
                        if self._is_valid_content(content):
                            self._cache[cache_key] = content
                            return content
                        print(
                            f"⚠️ Provider {name} returned invalid content "
                            f"(len={len(str(content).strip())})"
                        )
                        self._failed_providers[name] = {
                            "error": "invalid_content",
                            "status": "failed",
                        }
                        continue

                    error = str(result.get("content", ""))
                    print(f"⚠️ Provider failed: {name} -> {error[:200]}")
                    self._failed_providers[name] = {
                        "error": error,
                        "status": "failed",
                    }
                    continue

                elif result and self._is_valid_content(str(result)):
                    self._cache[cache_key] = result
                    return result

                else:
                    print(f"⚠️ Provider {name} returned empty/invalid result")
                    self._failed_providers[name] = {
                        "error": "empty_result",
                        "status": "failed",
                    }
                    continue

            except Exception as e:
                error = str(e)
                print(f"⚠️ Provider exception {name}: {error}")
                self._failed_providers[name] = {
                    "error": error,
                    "status": "exception",
                }
                continue

        fallback = (
            "تعذر إنشاء المحتوى بواسطة مزودي الذكاء الاصطناعي حالياً. "
            "يجب إعادة المحاولة بعد توفر المزود."
        )
        self._cache[cache_key] = fallback
        return fallback

    def health(self):
        return self.provider.health()

    def info(self):
        return {
            "manager": "Stable Provider Manager",
            "active_provider_name": self.provider.__class__.__name__,
            "failed_providers": self._failed_providers,
            "provider_details": (
                self.provider.info() if hasattr(self.provider, "info") else {}
            ),
        }
