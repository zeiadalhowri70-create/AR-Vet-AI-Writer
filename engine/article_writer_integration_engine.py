...
from engine.blogger_real_flow_adapter import BloggerRealFlowAdapter
from engine.blogger_gateway_engine import BloggerGatewayEngine
...
from engine.article_brain_attachment_engine import ArticleBrainAttachmentEngine
...
from engine.article_scientific_authority_engine import ArticleScientificAuthorityEngine
...
from engine.article_scientific_validation_engine import ArticleScientificValidationEngine
...
from engine.article_media_library_engine import ArticleMediaLibraryEngine
from core.media_pipeline_manager import MediaPipelineManager
from core.video_pipeline_adapter import VideoPipelineAdapter
...
class ArticleWriterIntegrationEngine:
    ...
    def __init__(self):
        self.real_flow_adapter = BloggerRealFlowAdapter()
        self.blogger_gateway = BloggerGatewayEngine()
        self.scientific_writer = ScientificArticleWriterEngine()
        self.scientific_validation = ArticleScientificValidationEngine()
        ...
        self.veterinary_brain_registry = VeterinaryBrainRegistryBinding()
        self.veterinary_brain = self.veterinary_brain_registry.build()["integration"]
        self.video_metadata = ArticleVideoMetadataEngine()
        self.media_library = ArticleMediaLibraryEngine()

        self.video_pipeline = VideoPipelineAdapter()

        self.media_pipeline = MediaPipelineManager(
            video_adapter=self.video_pipeline,
            enable_video=True
        )
        self.builder = ArticleRealContentBuilderEngine(
            veterinary_brain=self.veterinary_brain
        )
        ...
        self.html_writer = ArticleHTMLWriterEngine()
        self.faq = FAQEngine()
        ...
        self.article_brain_attachment = ArticleBrainAttachmentEngine()
        self.evidence_writer = ArticleEvidenceWriterEngine()
        self.quality_score = ArticleQualityScoreEngine()
        self.scientific_authority = ArticleScientificAuthorityEngine()
    ...
    def write(self, topic, context=None):
        """Compatibility bridge for WriterService pipeline."""
        return self.generate(topic)

    def generate(self, topic):
        # Compatibility Layer: accept dict topic requests
        if isinstance(topic, dict):
            topic = topic.get("disease", topic.get("topic", "unknown disease"))
        brain_result = self.veterinary_brain.execute(
            {"animal": "poultry", "disease": topic, "symptoms": []}
        )
        ...
        if resolved.get("status"):
            ...
            if isinstance(context, dict):
                context["veterinary_brain"] = brain_context

                context["integrated_intelligence"] = {
                    "knowledge": context.get("knowledge", {}),
                    "disease_profile": context.get("disease_profile", {}),
                    "veterinary_brain": brain_context,
                    "source": "AR-Vet AI Integrated Veterinary Intelligence",
                }
        ...
        try:
            ...
            try:
                scientific_review = self.scientific_validation.write(topic)

                if isinstance(context, dict):
                    context["scientific_validation"] = scientific_review
            ...
            if isinstance(context, dict):
                context["scientific_ai_draft"] = scientific_article

        except Exception:
            scientific_article = None
        ...
        article["veterinary_brain"] = brain_context

        if context and isinstance(context, dict):
            article["disease_profile"] = context.get(
                "disease_profile",
                context.get("disease", {})
            )
        ...
        if context and isinstance(context, dict):
            article["disease_profile"] = context.get(
                "disease_profile", context.get("disease", context)
            )
            article["context"] = context

        # Veterinary Brain Context Migration Layer
        if isinstance(context, dict):
            context["veterinary_brain"] = brain_context
        ...
        scientific_text = article.get("scientific_content", "")

        article["scientific_verification"] = {
            "status": "completed",
            "claim_verification": self.claim_verification.verify_all(
                [{"claim": scientific_text[:300], "references": []}]
            ),
            "fact_check": self.fact_checker.check_all(
                [{"fact": scientific_text[:300], "evidence": []}]
            ),
        }
        ...
        print(
            (
                len(article.get("sections", [{}])[0].get("content", "").split())
        ...
        if context and isinstance(context, dict):
            assembled["disease_profile"] = context.get(
                "disease_profile",
                context.get("disease", {})
            )

        # Scientific Evidence Context Bridge
        if context and isinstance(context, dict):
            if "disease_profile" in context:
                assembled["disease_profile"] = context["disease_profile"]
            elif "profile" in context:
                assembled["disease_profile"] = context["profile"]
            elif "disease" in context:
                assembled["disease_profile"] = context["disease"]
            else:
                assembled["disease_profile"] = {}
        ...
        profile = assembled.get("disease_profile", {})
        ...
        if isinstance(profile, dict):
            ...
            symptoms = scientific.get("clinical_signs", [])

        assembled["symptoms"] = symptoms

        assembled["veterinary_evidence"] = self.veterinary_evidence.analyze(
            profile, symptoms, assembled.get("context", {})
        )

        evidence_sources = []

        for ref in assembled.get("references", []):
            if isinstance(ref, dict):
                evidence_sources.append(ref)

        veterinary = assembled.get("veterinary_evidence", {})

        for test in veterinary.get("confirmation_tests", []):
            evidence_sources.append(
                {"organization": "Veterinary Evidence", "source": test}
            )

        for item in veterinary.get("evidence", []):
            if isinstance(item, dict):
                evidence_sources.append(
                    {
                        "organization": "Veterinary Evidence",
                        "source": item.get("feature"),
                    }
                )

        # Scientific Source Verification Gate
        try:
            assembled["source_verification"] = self.source_verification.verify_all(
                evidence_sources
            )
        except Exception as e:
            assembled["source_verification"] = {
                "valid": False,
                "error": str(e),
            }

        # Scientific Source Quality Gate
        try:
            assembled["source_quality"] = self.source_quality.evaluate_all(
                evidence_sources
            )
        except Exception as e:
            assembled["source_quality"] = {
                "average_score": 0,
                "error": str(e),
            }

        assembled["claim_evidence_analysis"] = self.claim_evidence.analyze_claim(
            f"المحتوى العلمي للمقال عن {topic}", evidence_sources
        )

        assembled["evidence_section"] = self.evidence_writer.write(
            topic, assembled.get("veterinary_evidence", {}).get("evidence", [])
        )
        ...
        assembled["seo"] = self.seo.enhance(
            assembled["seo_meta"]
        )
        ...
        assembled["media_library"] = self.media_library.build(
            topic
        )
...
"""
        ...
        # Video Intelligence Production Bridge
        try:
            assembled["media_intelligence"] = self.media_pipeline.build(
                assembled
            )

        except Exception as e:
            assembled["media_intelligence"] = {
                "status": False,
                "error": str(e),
            }


        html_body = assembled.get(
            "scientific_content",
            assembled.get("content", "")
        )

        html_body += "\n\n" + assembled.get("related_articles_html", "")
        html_body += "\n\n" + assembled.get("navigation_html", "")

        if isinstance(assembled.get("internal_links"), dict):
            html_body += "\n\n" + assembled["internal_links"].get("content", "")

        assembled["html"] = self.html_writer.render(
            title=assembled.get("title", topic),
            content=html_body,
            faq_html=assembled.get("faq_html", ""),
            canonical=assembled.get("canonical_url", ""),
            references=assembled.get("references", []),
            media_library=assembled.get("media_library", {}),
            image=assembled.get("media", {}).get("image", {}),
            video=(
                assembled.get("media_intelligence", {})
                .get("media", {})
                .get("video", {})
                or assembled.get("media", {}).get("video", {})
            ),
        )
        ...
        # Blogger Draft Integration (FINAL HTML)
        assembled["blogger_draft"] = self.blogger_gateway.prepare(assembled)
        assembled["documents"] = self.document_export.build(assembled)


        assembled["quality_score"] = self.quality_score.evaluate(assembled)

        assembled["scientific_authority"] = self.scientific_authority.evaluate(
            assembled
        )

        quality_data = dict(assembled["quality_score"])
        quality_data["final_score"] = quality_data.get(
            "score", quality_data.get("quality_score", 0)
        )

        assembled["confidence_calibration"] = self.confidence_calibration.calibrate(
            [quality_data]
        )

        # Complete the final data contract before final production gates.
        assembled["context"] = context

        if isinstance(context, dict):
            if "scientific_validation" in context:
                assembled["scientific_validation"] = context["scientific_validation"]

        if not assembled.get("references"):
            assembled["references"] = []

        # Brain attachment is part of the final production artifact.
        # It must exist before integrity/review/readiness/export.
        assembled = self.article_brain_attachment.attach(assembled)

        # Final artifact gates.
        try:
            assembled["integrity"] = self.integrity.check(assembled)
        except Exception as e:
            assembled["integrity"] = {
                "passed": False,
                "error": str(e),
        ...
        # Production identity/reporting after the artifact is complete.
        ...
        # Runtime completion is the final lifecycle transition.
        self.metrics.finish()
